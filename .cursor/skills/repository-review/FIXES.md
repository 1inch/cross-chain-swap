...
| `{{BUILD_COMMAND}}`, `{{TEST_COMMAND}}`, `{{LINT_COMMAND}}` | `package.json` scripts, `Makefile` targets, or the Foundry defaults `forge build` / `forge test` / `forge fmt --check` |
...
Agree the list once, then apply in the order below: `SECURITY.md` comes first because three later artifacts link to it. Each fix is written, stopped for user review, and committed only on an explicit yes — see `SKILL.md` step F3. Agreement on the list is not permission to commit.

### 1. `SECURITY.md` — closes §5

Copy `templates/SECURITY.md` to the repository root. The template already carries the org-wide disclosure channel (`security@1inch.com`), the HackenProof and Immunefi programme links (scope, payouts and SLAs), and the hiring link — do not blank those out. Prefill the audits section when the README already links audit reports or an `audits/` directory exists; leave the remaining TODO markers (supported deployments, audits if still empty, known limitations).

- **Precondition:** no `SECURITY.md` at the root, in `.github/` or in `docs/`.
- **Files:** `SECURITY.md`
- **Note:** this is the one file whose TODOs must lead the closing list. Contact, bounty and SLAs are filled by link; what remains is still a promise — what this repository supports, and its audits — which is why fixes land on a branch for review rather than on the default branch.

### 2. Issue and PR templates — closes §7
...
### 4. README repository-structure section — closes §1.4
...
### 5. README build and test section — closes §1.5

Insert the build/test snippet, naming the detected toolchain and the real commands. State the pinned solc version and, for Foundry, the pinned release.

- **Precondition:** `README.md` exists and names no build or test command.
- **Files:** `README.md`

### 6. README badges — closes §1.8, §1.9
...
### 7. Missing `README.md` — closes §1.1 partially

When there is no README at all, write the skeleton from `templates/readme-sections.md` with the structure, build and badge sections filled and a TODO for the protocol description. What the protocol does is not derivable from the file system.

- **Precondition:** no README at the root.
- **Files:** `README.md`

### 8. Mixed-licence note — closes §3.5
...
### 9. CI workflow — closes §6.1 to §6.4

With no CI at all, copy `templates/ci-foundry.yml` or `templates/ci-hardhat.yml` to `.github/workflows/ci.yml`. With CI present but a step missing, add the missing job to the existing workflow instead — a smaller and safer edit than replacing it.

- **Precondition:** no workflow runs build, tests or the linter. Resolve indirection first: `yarn test:ci`, `make test` and local composite actions all count.
...
### 10. Toolchain version pins in CI — closes §6.7
...
### 12. `deployments.md` — closes §8.5

When deployment records exist but no readable summary does, generate a table of network, contract, address and — where the record carries them — version, commit and date. Skip any record whose address cannot be resolved rather than guessing: a raw `CREATE3` broadcast names the deployer call, not the resulting contract.

- **Precondition:** `deployments/` exists with parseable records, and neither `deployments.md` nor a README deployment table exists.
- **Files:** `deployments.md`
...
### 14. Contracts directory renamed to `contracts/` — closes §2.1
...
### 15. Tests directory renamed to `tests/` — closes §2.2
...
### 16. Deployment scripts moved to `deploy/` — closes §2.3
...
### 17. Non-deployment scripts moved to `scripts/` — closes §2.5

Same as above, with the same classification step.

### 18. Documentation moved to `docs/` — closes §2.4
...
### 20. Fuzz and invariant tests in the CI run — closes §6.5
...
### 21. Compiler pins in the toolchain config — closes §A.4
...
### 22. `LICENSE` file when the SPDX headers are unanimous — closes §3.1, §3.2
...
**Needs data only the team has.** Audit reports and links (§5.5, §A.3); known limitations carried over from audits (§5.6); links to documentation, audits and the bounty in the README (§1.7) — the bounty URLs match the ones already in `SECURITY.md`; the 1inch logo asset when it is not already in the repository (§1.2); the Discord or forum link in `config.yml` (§7.5). The disclosure email, bug bounty programme links and response SLAs are org-wide defaults already in `templates/SECURITY.md` (SLAs live on the linked programme pages) — do not ask for them again.

**Needs a decision by the owner.** Which licence applies (§3.1, §3.2); BUSL Change Date and Change License (§3.4); which versions and deployments are supported (§5.4); upgrade governance — timelock, multisig, who may execute (§8.7); which linter to adopt when none is configured (§6.3).

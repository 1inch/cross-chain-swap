...
contract EscrowTest is BaseSetup {
    ...
    function test_TimelocksOverflow() public {
        vm.warp(1710159521); // make it real, it's 0 in foundry

        srcTimelocks = SrcTimelocks({ withdrawal: 120, cancellation: 1, publicCancellation: 2584807817 });
        ...
        skip(1); // 1 second later because dst.publicWithdrawal=src.withdrawal=1
        srcClone.publicCancel(immutables);

        assertEq(alice.addr.balance, balanceAliceNative + SRC_SAFETY_DEPOSIT + DST_SAFETY_DEPOSIT);
